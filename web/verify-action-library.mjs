...
assert.equal(new Set(manifest.map(d => d.id)).size, 19);
...
assert.deepEqual([...groups.values()], [10,3,3,3]);
assert.throws(() => parseActionClip({sensorCount:18,signal:[[1]],labels:[],times:[0]}));
console.log(`PASS: 19 entries (10 latest Human arm + 6 Direct + 3 contact voltage), ${frames} playback frames, correct sparse coordinates, timing and built assets; ${missing} missing measurements retained as null in source JSON`);
