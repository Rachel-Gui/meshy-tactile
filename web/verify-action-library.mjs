...
assert.equal(new Set(manifest.map(d => d.id)).size, 12);
...
console.log(`PASS: 12 actions (9 Direct + 3 contact voltage), ${frames} playback frames, correct sparse coordinates, timing and built assets; ${missing} missing measurements retained as null in source JSON`);
