...
SOURCE=ROOT/'human_arm/00_ALL_9_SCENES_FRONT_FACING_3X_SLOW_SMOOTH_MAPPING_FIXED_UPLOAD.csv'
...
def export():
 ...
 (SOURCE.parent/'latest-data-manifest.json').write_text(json.dumps({'source':str(SOURCE.relative_to(ROOT)),'sha256':sha,'frames':len(rows),'durationSeconds':times[-1],'sensorCount':96,'scenes':[{'label':label,'firstRow':start+2,'lastRow':end+1,'frames':end-start} for label,start,end in groups]},indent=2)+'\n')
