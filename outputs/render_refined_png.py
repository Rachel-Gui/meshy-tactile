...
for kind,script in [('finger',root/'finger-weave/render_styled_svg.py'),('arm',root/'arm-weave/styled/render_styled_svg.py')]:
 ...
 count,stations,start,end=(20,301,11,289) if kind=='finger' else (24,751,0,750)
 ...
 for band in range(2):
  ...
  for i in range(s):
   j=(i+1)%s
   for ids in [[o+i,o+j,o+s+j,o+s+i],[o+2*s+i,o+3*s+i,o+3*s+j,o+2*s+j],[o+i,o+2*s+i,o+2*s+j,o+j],[o+s+i,o+s+j,o+3*s+j,o+3*s+i]]:face(ids,[69,97,107])
 ...
 out=script.parent/f'{kind}-refined-hires-transparent.png'
 ...
 bg.convert('RGB').resize((W,H),Image.Resampling.LANCZOS).save(script.parent/f'{kind}-refined-check.png')
