...
LIBRARY_DIR = Path(__file__).resolve().parents[2]
...
def discover_entries() -> dict[str, list[Entry]]:
    ...
    thirty_path = (
        LIBRARY_DIR
        / "finger/action_library/30point_recordings_18node_sparse"
    ...
    for dataset, relative, kind in (
        ("18-point → sparse 6×6", "finger/action_library/18point_6x6_sparse/segments", "18_segment"),
        ("96-point → 12×8", "human_arm/action_library/96point_12x8/segments", "96_segment"),
        ("Robot arm 132-node → 11×12", "robot_arm/action_library/robot_arm_132node/segments", "132_segment"),
    ):
        entries = []
