#!/usr/bin/env python3
"""Select and browse every reviewed 30-, 18-, and 96-point action heatmap."""
...
def discover_entries() -> dict[str, list[Entry]]:
    ...
    for dataset, relative, kind in (
        ...
        ("96-point → 12×8", "../arm/96point_12x8/segments", "96_segment"),
    ):
        entries = []
        for path in sorted((LIBRARY_DIR / relative).glob("*.xlsx")):
            match = re.match(r"^(E\d+)_(front_touch|back_touch|grasp)_", path.name)
...
class HeatmapBrowser:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Finger sensor action heatmap library")
        ...
        self.dataset = ttk.Combobox(controls, state="readonly", width=20, values=list(self.entries))
